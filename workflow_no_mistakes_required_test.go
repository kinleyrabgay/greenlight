...
// TestGreenlightRequiredWorkflowExemptsReleaseAutomation pins the exemption
// logic so the release pipeline (release-please via GITHUB_TOKEN) and
// dependabot are never silently blocked by the gate.
func TestGreenlightRequiredWorkflowExemptsReleaseAutomation(t *testing.T) {
	data, err := os.ReadFile(".github/workflows/greenlight-required.yml")
	if err != nil {
...
// TestGreenlightRequiredWorkflowChecksSignatureMarker pins the exact signature
...
func TestGreenlightRequiredWorkflowChecksSignatureMarker(t *testing.T) {
	data, err := os.ReadFile(".github/workflows/greenlight-required.yml")
	if err != nil {
		t.Fatalf("read workflow: %v", err)
	}
	...
	marker := "Updates from [git push greenlight](https://github.com/kinleyrabgay/greenlight)"
...
// TestGreenlightRequiredWorkflowReadsPRBodyViaEnv pins the shell-injection-safe
// pattern: the PR body must be piped through an env var, not interpolated
// directly into the shell script body.
func TestGreenlightRequiredWorkflowReadsPRBodyViaEnv(t *testing.T) {
	data, err := os.ReadFile(".github/workflows/greenlight-required.yml")
	if err != nil {
		t.Fatalf("read workflow: %v", err)
	}
...
// TestGreenlightRequiredWorkflowTriggersOnRelevantPREvents ensures the check
// re-runs when the PR body is edited so a contributor cannot bypass by opening
// clean then editing the body.
func TestGreenlightRequiredWorkflowTriggersOnRelevantPREvents(t *testing.T) {
	data, err := os.ReadFile(".github/workflows/greenlight-required.yml")
	if err != nil {
		t.Fatalf("read workflow: %v", err)
	}
