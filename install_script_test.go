...
func TestInstallScriptInstallsUserOwnedBinaryAndPathSymlink(t *testing.T) {
	...
	home := t.TempDir()
	archivePath := filepath.Join(t.TempDir(), "greenlight-v1.2.3-darwin-arm64.tar.gz")
	binaryScript := "#!/bin/sh\nexit 0\n"
	...
	assertSymlinkTarget(t, filepath.Join(localBin, "greenlight"), realBin)
}
...
func TestInstallScriptReplacesExistingPathEntryWithSymlink(t *testing.T) {
	...
	oldPath := filepath.Join(linkDir, "greenlight")
...
func TestPowerShellInstallScriptChecksDaemonRestartFailure(t *testing.T) {
	...
	text := string(data)
	if !strings.Contains(text, "$restart = Start-Process -FilePath \"$installDir\\greenlight.exe\" -ArgumentList @(") {
...
func makeInstallArchive(t *testing.T, archivePath, binaryContent string) {
	...
	hdr := &tar.Header{Name: "greenlight", Mode: 0o755, Size: int64(len(data))}
