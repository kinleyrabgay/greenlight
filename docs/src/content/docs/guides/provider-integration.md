...
Without any provider setup, `greenlight` still gives you the local gate:
...
|---|---|---|---|
| **PR** (create/update) | `gh` CLI, authenticated | `glab` CLI, authenticated | `GREENLIGHT_BITBUCKET_EMAIL` + `GREENLIGHT_BITBUCKET_API_TOKEN` |
...
Once the host is wired up, `greenlight` can keep owning the branch after the
upstream push:
...
`greenlight doctor` also checks for `gh` availability.
For PR and workflow-run commands, greenlight passes the repository slug from the recorded upstream remote or PR URL to `gh`, so daemon-run commands do not depend on the daemon's current working directory.
...
```sh
export GREENLIGHT_BITBUCKET_EMAIL=you@example.com
export GREENLIGHT_BITBUCKET_API_TOKEN=your-api-token

# Optional: override the API base URL
export GREENLIGHT_BITBUCKET_API_BASE_URL=https://api.bitbucket.org/2.0
```
...
Self-hosted GitHub Enterprise and self-hosted GitLab instances work through the same `gh` and `glab` CLIs. Authenticate the CLI against your instance (`gh auth login --hostname your-ghe.example.com`, `glab auth login --hostname gitlab.example.com`) and `greenlight` will route through the CLI as usual.

## Unsupported hosts

If your upstream isn't GitHub, GitLab, or Bitbucket Cloud:

- The **push** step still runs - `greenlight` pushes through git like any other remote.
...
:::note
When the daemon runs through a managed service (launchd, systemd, Task Scheduler), it reloads environment from your login shell on macOS and Linux so `gh` auth and `GREENLIGHT_BITBUCKET_*` vars are picked up, and it augments `PATH` with common binary directories. If credentials or PATH-derived tools are missing, check `~/.greenlight/logs/daemon.log` for a login-shell environment resolution warning. On Windows it reuses the current process environment.
:::
