...
Configuration is optional. Without any config files, `greenlight` defaults to
...
- whether greenlight should infer intent from recent local agent transcripts

Config is split across two files:

| File | Scope |
|---|---|
| `~/.greenlight/config.yaml` | Global defaults for all repos |
| `<repo>/.greenlight.yaml` | Per-repo overrides |
...
See [Global Config Reference](/greenlight/reference/global-config/) for the full field listing.
...
- `GREENLIGHT_BITBUCKET_API_BASE_URL` - optional API base URL override

## Repo config

```yaml
# .greenlight.yaml (in repo root)
...
See [Repo Config Reference](/greenlight/reference/repo-config/) for the full field listing.
