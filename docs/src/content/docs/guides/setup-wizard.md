---
title: Setup Wizard
description: What bare `greenlight` does when there's no active run on the current branch.
---

When you run `greenlight` with no arguments and there is no active run on the
current branch, `greenlight` can walk you through creating a branch,
...
The point of the wizard is to make bare `greenlight` a useful starting
...
- You're in an interactive terminal, or you passed `greenlight -y` / `greenlight --yes`.
- The gate is initialized for the current repo (`greenlight init` has been run).
- There's no active run on the current branch.

In non-interactive contexts, bare `greenlight` without `-y` falls back to listing the last 5 runs instead.
...
Pass `--skip` to skip comma-separated pipeline steps for the new run created by the wizard, for example `greenlight --skip test,lint`.
It only applies when the wizard starts a new pipeline run; if bare `greenlight` attaches to an active run or lists recent runs, `--skip` exits with an error.

If you want to attach to *any* active run in the repo (not just the current branch), use `greenlight attach` - that path skips the wizard entirely.

## Wizard flow

```mermaid
flowchart TD
  start["Run greenlight"] --> active{"Active run on current branch?"}
...
Always shown. Asks whether to push the current branch to the `greenlight` gate.
...
The managed agent server (Rovo Dev or OpenCode) writes its output to `~/.greenlight/logs/wizard-agent.log` during these runs.

## Environment sanity

The wizard requires:

- The gate to be initialized (`greenlight init` has run).
...
`greenlight doctor` is the fastest way to check native agent availability.
