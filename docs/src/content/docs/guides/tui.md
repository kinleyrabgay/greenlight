...
`greenlight` or `greenlight attach`.
...
Bare `greenlight` can also open the [Setup Wizard](/greenlight/guides/setup-wizard/) first when there is no active run on the current branch and you need to create one.
...
The footer shows detach/help/yolo actions and, when `greenlight attach` has a cached newer release available, a right-aligned `<version> available` indicator. That update indicator stays visible after reruns in the same TUI session.
...
Press `q` to detach from the TUI. The pipeline continues running in the background. Run `greenlight` again to reattach to the active run on your current branch, or `greenlight attach` to reattach to the repo's active run without branch scoping.
