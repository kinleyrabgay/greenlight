...
First stop for anything: `greenlight doctor`.

## Debug in this order

```mermaid
flowchart TD
  problem["Something is wrong"] --> doctor["Run greenlight doctor"]
...
Symptoms: `greenlight daemon status` shows stopped, or `greenlight` exits with "daemon not running."
...
```sh
tail -f ~/.greenlight/logs/daemon.log
```
...
```sh
ls -la ~/.greenlight/daemon.pid ~/.greenlight/socket
```

If the PID file points at a process that's no longer running, remove both and run `greenlight daemon start` again.

### Managed service logs

- **macOS (launchd):** `launchctl list | grep greenlight` and check `~/Library/LaunchAgents/com.kinleyrabgay.greenlight.daemon.*.plist`
- **Linux (systemd):** `systemctl --user status greenlight-daemon-*` and `journalctl --user -u greenlight-daemon-* -f`
- **Windows (Task Scheduler):** `schtasks /query /tn "greenlight-daemon-*"`

### `NM_HOME` collisions

If you have multiple installs with different `NM_HOME` roots, each gets its own scoped service name (with a short suffix derived from the path). Make sure you're looking at the right one - `greenlight daemon status` reports which.

## `greenlight update` prompts or aborts
...
Pass `greenlight update -y` to confirm non-interactively while still printing warnings.
...
The daemon uses the same binary-discovery order described in [Choosing an Agent](/greenlight/guides/agents/). When it's running through a managed service, it reloads `PATH` from your login shell on macOS and Linux and appends common install locations such as `~/.local/bin`, `~/go/bin`, `~/.cargo/bin`, `~/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin`, and `/bin`.

If a native agent is installed in a version-manager shim directory or another nonstandard location, set an explicit override in `~/.greenlight/config.yaml`:
...
The daemon logs its effective `PATH` at startup in `~/.greenlight/logs/daemon.log` with the message `daemon environment ready`. If the log contains `login shell environment resolution failed` or `login shell environment resolution returned no entries`, the daemon used a degraded fallback `PATH` that may omit version-manager directories such as nvm, fnm, or volta, so tools like `pnpm` may be missing.
...
Requested test evidence may still be written under the managed temporary `greenlight-evidence` directory, or under the configured in-repo evidence directory when `test.evidence.store_in_repo` is enabled.
...
Check the per-step log at `~/.greenlight/logs/<runID>/<step>.log`.
...
## `git push greenlight` doesn't start a pipeline

Symptom: push succeeds but `greenlight` shows no active run.

### Check the remote

```sh
git remote -v | grep greenlight
```

If it's missing, run `greenlight init` again.
Re-running init refreshes an existing gate and repairs the `greenlight` remote when it is missing.
...
The hook should be executable. If it's missing or non-executable, `greenlight init` will reinstall it for an existing greenlight-managed gate.
For existing gate repos, `greenlight daemon restart` also installs missing greenlight-managed hooks and refreshes legacy managed hooks without overwriting custom hooks.
...
The hook talks to the daemon over `~/.greenlight/socket`. If the daemon isn't running, the push still succeeds (the hook never blocks), but no pipeline starts. Start the daemon and push again.

If the gate is older, re-running `greenlight init` or restarting the daemon also reapplies hook-path isolation for existing bare repos when Git supports `config --worktree`.
...
Check the [Provider Integration](/greenlight/guides/provider-integration/) requirements. Most common causes:
...
`ci_timeout` defaults to `4h`. Raise it in `~/.greenlight/config.yaml`:
...
You can approve (accept the risk), fix (run another auto-fix cycle), skip, or abort from the TUI or `greenlight axi respond`.

## Worktree won't clean up

Symptom: `~/.greenlight/worktrees/<repoID>/<runID>/` sticks around after a run ends.
...
```sh
greenlight daemon stop
rm -rf ~/.greenlight/worktrees ~/.greenlight/servers ~/.greenlight/socket ~/.greenlight/daemon.pid
greenlight daemon start
```

This keeps your gate repos, database, and config but clears transient state. For a full wipe, see the [Uninstall section](/greenlight/start-here/installation/#uninstall).

## Still stuck

- Check `~/.greenlight/logs/daemon.log` at `log_level: debug`
- File an issue: <https://github.com/kinleyrabgay/greenlight/issues>
