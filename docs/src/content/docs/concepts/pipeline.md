...
This page is the overview. For each step's exact behavior, defaults, skip rules, and fix-commit format, see [Pipeline Steps](/greenlight/reference/pipeline-steps/).
...
See [Auto-Fix Loop](/greenlight/concepts/auto-fix/) for how the fix cycle works, and [Using the TUI](/greenlight/guides/tui/) for what the approval UI looks like.
...
- Skip steps for one run with `greenlight --skip <steps>`, `git push -o greenlight.skip=<steps>`, `greenlight axi run --skip <steps>`, or from the TUI.

See [Configuration](/greenlight/guides/configuration/).
