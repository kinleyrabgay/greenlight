...
When a pipeline step finds issues, `greenlight` can automatically ask the agent to fix them before pausing for your approval. This is controlled by the `auto_fix` configuration.
...
Repo config overlays global config - you can set `auto_fix.lint: 5` in a repo's `.greenlight.yaml` to override just that step while inheriting the rest from global.
...
| Step | Commit prefix |
|---|---|
| Rebase | `greenlight(rebase): <summary>` |
| Review | `greenlight(review): <summary>` |
| Test | `greenlight(test): <summary>` |
| Document | `greenlight(document): <summary>` |
| Lint | `greenlight(lint): <summary>` |

The push step commits any remaining uncommitted changes with `greenlight: apply agent fixes`.
...
- `auto_fix` - also used when you press `f` in the TUI or use `greenlight axi respond --action fix` to run a follow-up fix
