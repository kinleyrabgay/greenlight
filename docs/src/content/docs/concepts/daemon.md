...
`greenlight`, `init`, `attach`, `rerun`, and `update` keep that service
installed and running for you when that path is available.

## Why a daemon exists

The daemon exists so `git push greenlight` stays fast and the gate can keep
...
```mermaid
flowchart LR
  push["git push greenlight"] --> gate["Gate repo hook"] --> daemon["Daemon"]
...
On macOS this is a per-user `launchd` agent, on Linux a per-user `systemd` service, and on Windows a Task Scheduler task. The installed artifact names are scoped by `NM_HOME` with a short stable suffix, so the paths and service identifiers look like `~/Library/LaunchAgents/com.kinleyrabgay.greenlight.daemon.<suffix>.plist`, `~/.config/systemd/user/greenlight-daemon-<suffix>.service`, and the Windows task `greenlight-daemon-<suffix>`. That keeps multiple `greenlight` installs from colliding when they use different `NM_HOME` roots. Those service managers keep the daemon available across CLI invocations and restart it after `greenlight update` replaces the binary. Before a managed daemon run starts, `greenlight` reloads the environment from your login shell on macOS and Linux and augments `PATH` with common user, Homebrew, and system binary directories so agent and tool discovery is not limited to the service manager's minimal environment. If login-shell resolution fails, it logs a warning and uses an augmented process-environment fallback that may omit version-manager directories. On Windows it reuses the current process environment instead of shelling out to a login shell. If managed service install or startup is unavailable or fails, `greenlight` falls back to starting a detached daemon process instead.
...
`greenlight update` stops and starts the daemon when it is running, or when stale daemon artifacts exist, so the new executable is used.
...
The daemon writes an identity record to `~/.greenlight/daemon.pid` and listens on a Unix socket at `~/.greenlight/socket`. On Windows, it uses a localhost TCP listener and a protected endpoint file at the same path.

## What it does

When a push arrives via the post-receive hook:

1. Creates a detached worktree at `~/.greenlight/worktrees/<repoID>/<runID>/`
...
- Refreshes legacy greenlight-managed `post-receive` hooks, installs missing managed hooks, and leaves custom hooks untouched
...
- Enables Git push-option support on existing gate repos so per-push options like `greenlight.skip=...` keep working after upgrades

## Logging

Daemon logs go to `~/.greenlight/logs/daemon.log`. The setup wizard captures managed agent-server output in `~/.greenlight/logs/wizard-agent.log`. Each pipeline step also writes to its own log at `~/.greenlight/logs/<runID>/<step>.log`, and fatal step errors are appended there so the step log includes the failure reason even when the detail comes from command stderr.
...
`greenlight daemon stop` stops the current daemon process without removing the managed service. The next `greenlight daemon start`, `greenlight`, `init`, `attach`, `rerun`, or `update` will start it again through the same service manager when available, or as a detached daemon otherwise.
