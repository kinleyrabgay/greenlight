...
This is the per-step reference. For the overview and rationale, see [Pipeline](/greenlight/concepts/pipeline/). For the fix loop, see [Auto-Fix Loop](/greenlight/concepts/auto-fix/).
...
The steering still allows requested test evidence under the managed temporary `greenlight-evidence` directory or the configured in-repo evidence directory, plus incidental temp or cache writes from normal development tools.
...
Fix commits use `greenlight(lint): <summary>`.
...
- Commits any uncommitted agent changes with message `greenlight: apply agent fixes`
...
- Bitbucket Cloud credentials are missing (`GREENLIGHT_BITBUCKET_EMAIL` or `GREENLIGHT_BITBUCKET_API_TOKEN`)
...
- Bitbucket Cloud requires `GREENLIGHT_BITBUCKET_EMAIL` and `GREENLIGHT_BITBUCKET_API_TOKEN`.
...
- While the PR stays open, the TUI and terminal title show `Checks passed` once checks are green and known mergeability is clear, and `greenlight axi` returns `outcome: checks-passed` with successful-output reporting instructions so agents can summarize the run, ask the user to review and merge, and list any pipeline fixes instead of waiting
