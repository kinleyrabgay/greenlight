...
description: All fields for ~/.greenlight/config.yaml.
---

Global configuration lives at `~/.greenlight/config.yaml`. Set `NM_HOME` to relocate the config directory.
...
When `agent: acp:<target>` matches an override key, greenlight runs `acpx --agent <command>` instead of `acpx <target>`.
...
When set, `greenlight` uses this path instead of looking up the binary on `PATH`.
...
User-supplied flags are inserted ahead of greenlight' managed flags, so your choices usually take precedence. A few flags are reserved because greenlight depends on them to communicate with the agent - setting any of these returns a config error on load:
...
For structured `codex` runs, greenlight also appends its own `--output-schema <tempfile>` after your overrides. Treat that flag as managed even though config validation does not currently reject it.
...
Permission and sandbox flags affect the underlying agent, but they do not disable greenlight' pipeline prompt steering.
...
When enabled and no intent was supplied directly for the run, greenlight can read recent local agent transcripts, match the session that produced the change, summarize the author's intent, pass that summary to rebase, review, test, document, lint, CI auto-fix, and PR prompts, and include it in generated PR descriptions.
...
For multi-file diffs, greenlight still requires at least two overlapping files and an effective minimum score of `0.5`.
...
| `test.evidence.dir` | `string` | `.greenlight/evidence` | Repo-relative parent directory used when `store_in_repo` is true |
...
If `dir` is absolute, escapes the worktree, points into `.git`, crosses a symlink, or is ignored by Git, greenlight falls back to temporary evidence storage for that run.

These are global defaults. Per-repo config can override either field.

## Environment variables

See [Environment Variables](/greenlight/reference/environment/) for `NM_HOME`, Bitbucket Cloud credentials, and update-check suppression.
