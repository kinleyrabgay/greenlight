...
import (
	...
	gitpkg "github.com/kinleyrabgay/greenlight/internal/git"
...
func TestInit(t *testing.T) {
	...
	// Verify greenlight remote was added to working repo.
	url, err := gitpkg.GetRemoteURL(ctx, workDir, "greenlight")
	if err != nil {
...
func TestInitIsIdempotent(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, workDir, RemoteName); err != nil {
		t.Errorf("greenlight remote missing after re-init: %v", err)
	} else if url != bareDir {
		t.Errorf("remote url = %q, want %q", url, bareDir)
	}
	dbRepo, err := d.GetRepoByPath(workDir)
...
func TestInitRefreshUsesPersistedRepoID(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("get greenlight remote: %v", err)
	}
	if url != legacyBareDir {
		t.Errorf("greenlight remote = %q, want %q", url, legacyBareDir)
	}
...
func TestInitRepairsBrokenGate(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, workDir, RemoteName); err != nil {
		t.Errorf("greenlight remote not restored after repair re-init: %v", err)
	} else if url != bareDir {
...
// a working directory does not break init idempotency: the leftover greenlight
...
func TestInitReattachesGateAfterWorkingDirRename(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, renamed, RemoteName); err != nil {
		t.Errorf("greenlight remote missing after reattach: %v", err)
	} else if url != bareDir {
		t.Errorf("remote url = %q, want %q", url, bareDir)
	}
...
// new repo: it gets its own gate and the copied greenlight remote is
...
func TestInitCreatesFreshGateForCopiedWorkingDir(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, copyDir, RemoteName); err != nil {
		t.Errorf("greenlight remote missing on copy: %v", err)
	...
	if url, err := gitpkg.GetRemoteURL(ctx, workDir, RemoteName); err != nil {
		t.Errorf("original greenlight remote missing: %v", err)
...
// greenlight remote pointing into our repos dir with no matching DB record
...
func TestInitRepointsOrphanGateRemoteOnFreshInit(t *testing.T) {
	...
	if url, err := gitpkg.GetRemoteURL(ctx, renamed, RemoteName); err != nil {
		t.Errorf("greenlight remote missing: %v", err)
	} else if url != p.RepoDir(repo.ID) {
		t.Errorf("remote url = %q, want %q", url, p.RepoDir(repo.ID))
	}
}

func TestInitDoesNotOverwriteExistingGreenlightRemoteOnFreshInit(t *testing.T) {
	workDir := setupTestRepo(t)
	nmRoot := t.TempDir()
	p := paths.WithRoot(nmRoot)
	if err := p.EnsureDirs(); err != nil {
	...
	if err == nil {
		t.Fatal("expected init to fail when greenlight remote already exists")
	}
	...
	if url != customRemote {
		t.Errorf("greenlight remote = %q, want %q", url, customRemote)
	}
}
...
func TestEject(t *testing.T) {
	...
	_, err = gitpkg.GetRemoteURL(ctx, workDir, "greenlight")
	if err == nil {
		t.Error("expected greenlight remote to be removed")
	}
...
func TestInit_PostReceiveSurvivesHooksPathPoisoning(t *testing.T) {
	...
	if out, err := exec.Command("git", "-C", workDir, "push", "greenlight", "HEAD:refs/heads/test-branch").CombinedOutput(); err != nil {
		t.Fatalf("push: %v: %s", err, out)
	}
