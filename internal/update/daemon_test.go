...
func TestEnsureDaemonUsesCurrentExecutableAllowsWindowsCaseDifferences(t *testing.T) {
	...
	daemonExecutablePath = func(*paths.Paths) (string, error) {
		return `c:\program files\greenlight\GREENLIGHT.exe`, nil
	}

	u := &updater{
		executablePath: `C:\Program Files\Greenlight\greenlight.exe`,
...
func TestRunningDaemonExecutablePathHandlesExecutablePathsWithSpaces(t *testing.T) {
	if os.Getenv("GREENLIGHT_TEST_CHILD") == "1" {
	...
	copyPath := filepath.Join(dir, "greenlight test binary"+filepath.Ext(originalPath))
	...
	cmd.Env = append(os.Environ(), "GREENLIGHT_TEST_CHILD=1")
...
func TestExecutablePathForPIDUsesWindowsResolver(t *testing.T) {
	...
	if got != `C:\Program Files\greenlight\greenlight.exe` {
