...
func TestUpdaterCheckLatestAndRefreshCache(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			archiveName: "greenlight-v1.2.3-darwin-arm64.tar.gz",
		},
		{
			name:        "windows zip",
			platform:    platformSpec{GOOS: "windows", GOARCH: "amd64"},
			archiveName: "greenlight-v1.2.3-windows-amd64.zip",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.URL.Path != "/repos/kinleyrabgay/greenlight/releases/latest" {
...
func TestUpdaterRunReplacesExecutable(t *testing.T) {
	...
	archive := makeTarGz(t, map[string][]byte{
		"bin/greenlight": []byte("new-binary"),
	})
	...
	server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/kinleyrabgay/greenlight/releases/latest":
	...
	execPath := filepath.Join(t.TempDir(), "greenlight")
	...
	if !strings.Contains(stdout.String(), "updated greenlight") {
...
func TestUpdaterRunFailsWhenDaemonUsesDifferentExecutable(t *testing.T) {
	...
	execDir := t.TempDir()
	execPath := filepath.Join(execDir, "greenlight")
	if err := os.WriteFile(execPath, []byte("old-binary"), 0o755); err != nil {
		t.Fatal(err)
	}
	otherExecPath := filepath.Join(execDir, "other-greenlight")
...
func TestUpdaterRunReplacesDaemonWhenDifferentExecutableConfirmed(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !strings.Contains(stdout.String(), "updated greenlight from v1.2.2 to v1.2.3") {
...
func TestUpdaterMaybeNotifyAndCheck(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "A new version of greenlight is available: v1.2.2 -> v1.2.3") {
		t.Fatalf("stderr = %q", stderr.String())
	}
	if !spawned {
...
func TestUpdaterCheckLatestBetaUsesReleasesList(t *testing.T) {
	...
	archiveName := "greenlight-v1.3.0-beta.1-darwin-arm64.tar.gz"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/kinleyrabgay/greenlight/releases":
			fmt.Fprintf(w, `[
		...
		case "/repos/kinleyrabgay/greenlight/tags":
...
func TestUpdaterCheckLatestBetaPicksHighestSemver(t *testing.T) {
	...
	archiveName := "greenlight-v1.3.0-beta.2-darwin-arm64.tar.gz"
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/repos/kinleyrabgay/greenlight/releases":
			fmt.Fprintf(w, `[
...
func TestUpdaterCheckLatestBetaFallsBackToTagsWhenListingStale(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/repos/kinleyrabgay/greenlight/releases/tags/v1.3.0-beta.1":
...
func TestUpdaterCheckLatestBetaChecksListedReleaseAfterMissingTags(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		default:
			if strings.HasPrefix(r.URL.Path, "/repos/kinleyrabgay/greenlight/releases/tags/") {
				tagFetches++
				http.NotFound(w, r)
				return
			}
