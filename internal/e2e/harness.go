...
// greenlight binary against a fake agent. Each test gets its own Harness;
...
// NewHarness builds the greenlight + fakeagent binaries (once per test
// process), creates a temp git repo with origin, writes the greenlight
...
func NewHarness(t *testing.T, opts SetupOpts) *Harness {
	...
	// test, so subprocesses spawned by greenlight inherit these. The
	...
	t.Setenv("GREENLIGHT_TELEMETRY", "off")
	...
	t.Setenv("GREENLIGHT_NO_UPDATE_CHECK", "1")
...
// writeGlobalConfig writes a greenlight global config that pins the
// agent name and binary path. The path override forces greenlight'
...
func (h *Harness) initGitRepos() {
	...
	repoConfig := filepath.Join(h.WorkDir, ".greenlight.yaml")
	if err := os.WriteFile(repoConfig, []byte("ignore_patterns:\n  - '*.generated.go'\n  - 'vendor/**'\n"), 0o644); err != nil {
		h.t.Fatalf("write repo config: %v", err)
	}
	mustGit(h.WorkDir, "add", "README.md", ".greenlight.yaml")
...
// Run invokes the greenlight binary in the working repo and returns
...
// RunInDir invokes the greenlight binary in dir and returns stdout+stderr.
func (h *Harness) RunInDir(dir string, args ...string) (string, error) {
	h.t.Helper()
	return h.RunInDirWithEnv(dir, nil, args...)
}

// RunInDirWithEnv invokes the greenlight binary in dir with env overrides.
...
// PushToGate pushes the current branch through the greenlight remote,
...
func (h *Harness) PushToGate(branch string) {
	...
	if err != nil {
		h.t.Fatalf("git push greenlight %s: %v\n%s", branch, err, out)
	}
}
...
// buildBinaries compiles the greenlight binary and the fakeagent binary
...
func buildBinaries(t *testing.T) (nmBin, fakeBin string) {
	...
	buildOnce.Do(func() {
		...
		nm := filepath.Join(dir, executableName("greenlight"))
...
// the go.mod that declares the greenlight module. Robust to test
