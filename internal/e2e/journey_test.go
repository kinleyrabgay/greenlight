...
//
//   - `greenlight init` (gate setup, daemon bootstrap, post-receive
//     hook installation)
//   - `git push greenlight <branch>` (real git transport, hook fires,
...
func runHappyPath(t *testing.T, agentName string) {
	...
	// `greenlight init` sets up the gate and starts the daemon.
...
func assertRootVersion(t *testing.T, h *Harness) {
	...
	if !strings.HasPrefix(out, "greenlight version ") {
...
func assertInitOutput(t *testing.T, h *Harness, out string) {
	...
	for _, want := range []string{resolved, "git push greenlight", "|__| |_/", "Gate initialized"} {
		if !strings.Contains(out, want) {
...
func assertRunsEmpty(t *testing.T, h *Harness) {
	...
	for _, want := range []string{"no runs", "git push greenlight <branch>"} {
		if !strings.Contains(out, want) {
...
func assertRootNoActiveRun(t *testing.T, h *Harness) {
	...
	for _, want := range []string{"No active run", "git push greenlight"} {
		if !strings.Contains(out, want) {
...
func assertRootNoActiveRunOnOtherBranch(t *testing.T, h *Harness, activeRun *ipc.RunInfo) {
	...
	for _, want := range []string{"No active run", "Recent runs", activeRun.Branch, string(activeRun.Status), "git push greenlight"} {
		if !strings.Contains(out, want) {
...
func assertRootRecentRuns(t *testing.T, h *Harness, run *ipc.RunInfo) {
	...
	for _, want := range []string{"No active run", "Recent runs", run.Branch, string(run.Status), sha, "git push greenlight"} {
		if !strings.Contains(out, want) {
...
func assertAgentEditCommitRun(t *testing.T, h *Harness) {
	...
	originalHead := h.CommitChange("agent-edits", ".greenlight.yaml", config, "configure formatter")
	...
	if strings.TrimSpace(string(message)) != "greenlight: apply agent fixes" {
...
func assertFormatFailureWarningRun(t *testing.T, h *Harness) {
	...
	config := "ignore_patterns:\n  - '*.generated.go'\n  - 'vendor/**'\ncommands:\n  format: \"exit 1\"\n"
	h.CommitChange("format-fails", ".greenlight.yaml", config, "configure failing formatter")
...
func assertGateRefDeletionDoesNotCreateRun(t *testing.T, h *Harness, branch string) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	out, err := h.runGit(ctx, h.WorkDir, "push", "greenlight", ":"+branch)
	if err != nil {
...
func assertConfiguredCommandRun(t *testing.T, h *Harness) {
	...
	config := "ignore_patterns:\n  - '*.generated.go'\n  - 'vendor/**'\ncommands:\n  test: nm-test-e2e\n  lint: nm-lint-e2e\n"
	head := h.CommitChange("configured-commands", ".greenlight.yaml", config, "enable configured checks")
...
func assertExplicitAttachUsesRepoWideActiveRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("explicit-attach-active", ".greenlight.yaml", config, "configure explicit attach slow test")
...
func assertSupersededRunCancellation(t *testing.T, h *Harness) {
	...
	h.CommitChange("superseded-run", ".greenlight.yaml", config, "configure superseded slow test")
...
func assertDifferentBranchDoesNotCancelActiveRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("different-branch-slow", ".greenlight.yaml", slowConfig, "configure different-branch slow test")
	...
	h.CommitChange("different-branch-fast", ".greenlight.yaml", fastConfig, "configure different-branch fast checks")
...
func assertCancelRunStopsActivePipeline(t *testing.T, h *Harness) {
	...
	h.CommitChange("cancel-run", ".greenlight.yaml", config, "configure cancel slow test")
...
func assertRespondNoWaitingStepRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("respond-no-waiting", ".greenlight.yaml", config, "configure slow test command")
...
func assertFailingTestCommandRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("failing-test-command", ".greenlight.yaml", config, "configure failing test command")
...
func assertFailingLintCommandRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("failing-lint-command", ".greenlight.yaml", config, "configure failing lint command")
...
func assertRootDefaultsToHistory(t *testing.T, h *Harness) {
	...
	if !strings.Contains(out, "more - run 'greenlight runs' to see all") {
...
func assertGateRemotePresent(t *testing.T, h *Harness) {
	t.Helper()
	out, err := h.runGit(context.Background(), h.WorkDir, "remote", "get-url", "greenlight")
	if err != nil {
		t.Fatalf("greenlight remote not found: %v\n%s", err, out)
	}
	want := filepath.Join(h.NMHome, "repos", h.repoID()+".git")
	if strings.TrimSpace(string(out)) != want {
		t.Errorf("greenlight remote URL = %q, want %q", strings.TrimSpace(string(out)), want)
	}
}
...
func assertGateRemoteAbsent(t *testing.T, h *Harness) {
	...
	out, err := h.runGit(context.Background(), h.WorkDir, "remote", "get-url", "greenlight")
	if err == nil {
		t.Fatalf("greenlight remote should have been removed after eject, got %s", out)
	}
}
