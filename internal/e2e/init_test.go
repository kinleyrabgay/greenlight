...
func TestInitIsIdempotent(t *testing.T) {
	...
	skillPath := filepath.Join(h.HomeDir, ".claude", "skills", "greenlight", "SKILL.md")
	...
	// The greenlight remote must still be wired after the refresh.
	if out, err := h.runGit(context.Background(), h.WorkDir, "remote", "get-url", "greenlight"); err != nil {
		t.Fatalf("greenlight remote missing after re-init: %v\n%s", err, out)
	}
}

// TestInitLegacyNotice proves init in a repo that still carries a vendored
// skill copy from an older greenlight version points it out without touching
...
func TestInitLegacyNotice(t *testing.T) {
	...
	legacy := filepath.Join(h.WorkDir, ".agents", "skills", "greenlight", "SKILL.md")
	...
	if !strings.Contains(out, filepath.Join(".agents", "skills", "greenlight", "SKILL.md")) {
...
func TestInitRepoRename(t *testing.T) {
	...
	gateURLOut, err := h.runGit(ctx, h.WorkDir, "remote", "get-url", "greenlight")
	if err != nil {
	...
	if out, err := h.runGit(ctx, renamed, "remote", "get-url", "greenlight"); err != nil {
		t.Fatalf("greenlight remote missing after rename re-init: %v\n%s", err, out)
...
func TestInitRollsBackWhenDaemonStartFails(t *testing.T) {
	...
	ctx := context.Background()
	if out, err := h.runGit(ctx, h.WorkDir, "remote", "get-url", "greenlight"); err == nil {
		t.Fatalf("greenlight remote should be removed after failed init, got %q", out)
	}
