...
import (
	...
	"github.com/kinleyrabgay/greenlight/internal/cimonitor"
...
func newAxiRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Triggers a pipeline run for the current branch and drives it. Without\n" +
			...
			"so greenlight uses it directly instead of inferring it from transcripts.",
		Args:          cobra.NoArgs,
		SilenceErrors: true,
		SilenceUsage:  true,
	...
	if runID == "" {
		...
		if strings.TrimSpace(intent) == "" {
			return emitError(cmd, 2, "--intent is required to start a run",
				`Pass what the user set out to accomplish: greenlight axi run --intent "the user's goal"`)
		}
...
func runAxiRespond(cmd *cobra.Command, ra respondArgs) error {
	...
	case "":
		return emitError(cmd, 2, "--action is required",
			"Run `greenlight axi respond --action approve|fix|skip`")
	default:
	...
	if active.Run == nil {
		return emitError(cmd, 1, "no active run to respond to",
			"Run `greenlight axi run` to start one")
	}
	runID := active.Run.ID
	...
	if stepName == "" {
		...
		if !ok {
			return emitError(cmd, 1, "no step is awaiting approval",
				"Run `greenlight axi status` to see the run state")
		}
	...
	if act == types.ActionFix {
		if len(findingIDs) == 0 && ra.addFinding == "" {
			return emitError(cmd, 2, "--action fix requires --findings <id,...> or --add-finding <json>",
				"Run `greenlight axi status` to list finding IDs")
		}
