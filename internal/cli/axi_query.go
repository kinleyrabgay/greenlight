...
func startRunHelp() string {
	return `Run greenlight axi run --intent "the user's goal" --yes to validate the current branch`
}
...
func runAxiLogs(cmd *cobra.Command, step, runID string, full bool) error {
	...
	if !full && len(lines) > logTailLines {
		...
		fields = append(fields,
			...
			toon.Field{Key: "help", Value: []string{fmt.Sprintf("Run `greenlight axi logs --step %s --full` to see the entire log", step)}},
		)
		emitDoc(cmd, fields...)
		return nil
	}
