...
func TestStatsCommandRendersAllRepoDashboard(t *testing.T) {
	...
	for _, want := range []string{
		"╭─ git push greenlight",
		"____ ____ ____ ____ _  _ _    _ ____ _  _ ___",
		"Total changes",
		"Rescued changes",
		"Rescue rate",
		"50%",
	...
	for _, notWant := range []string{"Saved", "Rescue runs", "Mistakes fixed", "auto-fix", "caught in review", "╭─ greenlight"} {
...
func TestStatsDashboardTopBorderShowsGitPushGreenlight(t *testing.T) {
	out := renderStatsDashboard(&db.Stats{})
	firstLine := strings.Split(out, "\n")[0]
	if !strings.Contains(firstLine, "git push greenlight") {
...
func TestStatsDashboardCentersBannerAsBlock(t *testing.T) {
	...
	for _, line := range lines {
		if strings.Contains(line, "____ ____ ____ ____") || strings.Contains(line, `| __ |__/ |___`) || strings.Contains(line, `|__] |  \ |___`) {
...
func TestStatsDashboardStylesBannerAndProgressBars(t *testing.T) {
	...
	if !strings.Contains(out, sCyan.Render("____ ____ ____ ____ _  _ _    _ ____ _  _ ___")) {
