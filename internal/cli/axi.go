...
// /greenlight skill) drives; humans use the bare `greenlight` TUI instead.
func newAxiCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "axi",
		Short: "Agent interface: drive greenlight from an autonomous agent",
		Long: "Agent eXperience Interface for greenlight. Prints token-efficient TOON\n" +
			"to stdout and is driven entirely by flags (no interactive prompts).\n" +
			"Running `greenlight axi` with no subcommand shows the current state.",
		Args:          cobra.NoArgs,
		SilenceErrors: true,
		SilenceUsage:  true,
...
func runAxiHome(cmd *cobra.Command) error {
	...
	case currentActive == nil:
		help = append(help, `Run `+"`"+`greenlight axi run --intent "<what the user set out to accomplish>"`+"`"+` to validate your changes`)
		if otherActive != nil {
	...
	case gated:
		help = append(help, "Run `greenlight axi respond --action approve` to clear the current gate")
	default:
		help = append(help, "Run `greenlight axi status` to inspect the active run")
	}
...
func repoInitHelp(err error) []string {
	if err != nil && strings.Contains(err.Error(), "not initialized") {
		return []string{"Run `greenlight init` to set up the gate in this repository"}
	}
	return nil
}
