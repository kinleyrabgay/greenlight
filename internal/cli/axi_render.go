...
func gateFields(gate stepView) []toon.Field {
	...
	return []toon.Field{
		...
		{Key: "help", Value: []string{
			"Run `greenlight axi respond --action approve` to accept this step and continue",
			"Run `greenlight axi respond --action fix --findings <ids>` to have the pipeline fix the selected findings (do not edit files yourself)",
			"Run `greenlight axi respond --action skip` to skip this step",
			fmt.Sprintf("Run `greenlight axi logs --step %s --full` to read the full step log", gate.Name),
		}},
	}
}
