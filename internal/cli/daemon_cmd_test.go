...
func TestParseSkipPushOptionsRejectsUnknownStep(t *testing.T) {
	_, err := parseSkipPushOptions([]string{"greenlight.skip=test,deploy"})
	if err == nil {
...
func TestFormatSkipPushOptions(t *testing.T) {
	...
	want := []string{"greenlight.skip=test,lint"}
	if !reflect.DeepEqual(got, want) {
...
func TestIntentPushOptionRoundTrip(t *testing.T) {
	...
	got, err := parseIntentPushOptions([]string{"greenlight.skip=test", opt})
	if err != nil {
...
func TestParseIntentPushOptionsNone(t *testing.T) {
	got, err := parseIntentPushOptions([]string{"greenlight.skip=test", "ci.skip"})
	if err != nil {
		t.Fatalf("parseIntentPushOptions() error = %v", err)
	}
	if got != "" {
