...
func newDaemonCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "daemon",
		Short: "Manage the greenlight daemon",
	}
...
func parseSkipPushOptions(options []string) ([]types.StepName, error) {
	...
	for _, option := range options {
		value, ok := strings.CutPrefix(option, "greenlight.skip=")
		if !ok {
			continue
		}
...
const intentPushOptionPrefix = "greenlight.intent="
...
func formatSkipPushOptions(steps []types.StepName) []string {
	...
	return []string{"greenlight.skip=" + strings.Join(parts, ",")}
}
...
func newDaemonRunCmd() *cobra.Command {
	...
	cmd.Flags().StringVar(&root, "root", "", "override greenlight data directory")
	return cmd
}
