...
func TestStartInstallsWindowsTaskAndStartsManagedDaemon(t *testing.T) {
	...
	defer cleanup()
	runtimeGOOS = "windows"
	exe := `C:\Program Files\greenlight\greenlight.exe`
...
func TestInstallWindowsTaskDoesNotRemoveLegacyTaskForDifferentRoot(t *testing.T) {
	...
	if err := installWindowsTask(p, `C:\Program Files\greenlight\greenlight.exe`); err != nil {
		t.Fatal(err)
	}
	if len(commands) != 2 {
...
func TestInstallWindowsTaskKeepsLegacyTaskOnCreateFailure(t *testing.T) {
	...
	err := installWindowsTask(p, `C:\Program Files\greenlight\greenlight.exe`)
	if err == nil {
