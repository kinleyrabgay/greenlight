...
func TestStart_ReinstallsManagedServiceWhenPlistChanged(t *testing.T) {
	...
	serviceExecutablePath = func() (string, error) { return "/opt/greenlight/bin/greenlight", nil }
...
func TestStart_DoesNotReinstallWhenPlistUnchanged(t *testing.T) {
	...
	current := renderLaunchAgent("/opt/greenlight/bin/greenlight", p, home)
...
func TestStartDoesNotRestartLaunchAgentForExecutableOnlyChange(t *testing.T) {
	...
	serviceUserHomeDir = func() (string, error) { return home, nil }
	serviceCurrentUser = func() (*user.User, error) { return &user.User{Uid: "501"}, nil }
	serviceExecutablePath = func() (string, error) { return "/private/var/folders/go-build/greenlight", nil }

	plistPath := filepath.Join(home, "Library", "LaunchAgents", launchdServiceLabel(p)+".plist")
	if err := os.MkdirAll(filepath.Dir(plistPath), 0o755); err != nil {
		t.Fatal(err)
	}
	installed := renderLaunchAgent("/opt/greenlight/bin/greenlight", p, home)
...
func TestStartPreservesInstalledExecutableWhenRefreshingLaunchAgent(t *testing.T) {
	...
	stale := renderLaunchAgentWithoutEnvironment("/opt/greenlight/bin/greenlight", p)
	...
	plist := string(data)
	if !strings.Contains(plist, "<string>/opt/greenlight/bin/greenlight</string>") {
		t.Fatalf("expected refreshed plist to preserve installed executable, got:\n%s", plist)
	}
	if strings.Contains(plist, "/private/var/folders/go-build/greenlight") {
...
func TestStartRestartsSystemdUnitWhenDefinitionChanged(t *testing.T) {
	...
	if err := os.WriteFile(unitPath, []byte("[Service]\nExecStart=/old/greenlight daemon run\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestManagedServiceInstalledRequiresMatchingRoot(t *testing.T) {
	...
	if err := os.WriteFile(plistPath, []byte(renderLaunchAgent("/opt/greenlight/bin/greenlight", paths.WithRoot(otherRoot), home)), 0o644); err != nil {
		t.Fatal(err)
	}
