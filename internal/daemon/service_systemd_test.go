...
func TestStartInstallsSystemdUnitAndStartsManagedDaemon(t *testing.T) {
	...
	for _, want := range []string{
		"Description=greenlight background daemon",
		"ExecStart=/usr/local/bin/greenlight daemon run --root " + p.Root(),
...
func TestInstallSystemdUserServiceDoesNotRemoveLegacyUnitForDifferentRoot(t *testing.T) {
	...
	legacyUnit := renderSystemdUnit("/usr/local/bin/greenlight", paths.WithRoot(otherRoot), home)
	...
	if err := installSystemdUserService(p, "/usr/local/bin/greenlight"); err != nil {
		t.Fatal(err)
	}
...
func TestInstallSystemdUserServiceKeepsLegacyUnitOnEnableFailure(t *testing.T) {
	...
	err := installSystemdUserService(p, "/usr/local/bin/greenlight")
	if err == nil {
