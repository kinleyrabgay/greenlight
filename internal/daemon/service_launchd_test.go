...
func TestStartInstallsLaunchAgentAndBootstrapsManagedDaemon(t *testing.T) {
	...
	for _, want := range []string{
		"<string>/opt/greenlight/bin/greenlight</string>",
...
func TestInstallLaunchAgentKeepsLegacyPlistOnScopedWriteFailure(t *testing.T) {
	...
	err := installLaunchAgent(p, "/opt/greenlight/bin/greenlight")
	if err == nil {
...
func TestInstallLaunchAgentDoesNotRemoveLegacyPlistForDifferentRoot(t *testing.T) {
	...
	legacyPlist := renderLaunchAgent("/opt/greenlight/bin/greenlight", paths.WithRoot(otherRoot), home)
	...
	if err := installLaunchAgent(p, "/opt/greenlight/bin/greenlight"); err != nil {
		t.Fatal(err)
	}
