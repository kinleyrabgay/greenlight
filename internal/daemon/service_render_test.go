...
import (
	...
	"github.com/kinleyrabgay/greenlight/internal/shellenv"
)
...
func TestServiceDefinitionMatchesRootRejectsPrefixOnlyMatch(t *testing.T) {
	...
	if serviceDefinitionMatchesRoot([]byte(renderLaunchAgent("/opt/greenlight/bin/greenlight", other, home)), p) {
		t.Fatal("expected launch agent root prefix collision to be rejected")
	}
	if serviceDefinitionMatchesRoot([]byte(renderSystemdUnit("/usr/local/bin/greenlight", other, home)), p) {
	...
	if !serviceDefinitionMatchesRoot([]byte(renderLaunchAgent("/opt/greenlight/bin/greenlight", p, home)), p) {
		t.Fatal("expected exact launch agent root match")
	}
	if !serviceDefinitionMatchesRoot([]byte(renderSystemdUnit("/usr/local/bin/greenlight", p, home)), p) {
...
func TestRenderLaunchAgentIncludesManagedPath(t *testing.T) {
	...
	plist := renderLaunchAgent("/opt/greenlight/bin/greenlight", p, home)
...
func TestRenderSystemdUnitIncludesManagedPath(t *testing.T) {
	...
	unit := renderSystemdUnit("/usr/local/bin/greenlight", p, home)
