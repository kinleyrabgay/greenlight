...
func TestClientTrackSendsUmamiEventPayload(t *testing.T) {
	...
	case got := <-reqCh:
		...
		if got.Payload.URL != "app://greenlight/command" {
			t.Fatalf("url = %q, want %q", got.Payload.URL, "app://greenlight/command")
		}
		if got.Payload.Title != "greenlight CLI" {
			t.Fatalf("title = %q, want %q", got.Payload.Title, "greenlight CLI")
		}
