...
import (
	...
	"github.com/kinleyrabgay/greenlight/internal/buildinfo"
)
...
func TestDefaultUsesDotEnvInDevBuildWhenEnvMissing(t *testing.T) {
	...
	content := "GREENLIGHT_UMAMI_HOST=https://dotenv.example\nGREENLIGHT_UMAMI_WEBSITE_ID=website-from-dotenv\n"
...
func TestDefaultDisablesTelemetryWhenEnvIsOff(t *testing.T) {
	...
	if _, ok := Default().(*Client); ok {
		t.Fatal("Default() should disable telemetry when GREENLIGHT_TELEMETRY=off")
	}
}
...
func TestDefaultIgnoresDotEnvOutsideRepo(t *testing.T) {
	...
	parentDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(parentDir, ".env"), []byte("GREENLIGHT_UMAMI_WEBSITE_ID=outside-repo\n"), 0o644); err != nil {
...
func TestParseDotEnvStripsInlineCommentsFromUnquotedValues(t *testing.T) {
	values := parseDotEnv([]byte("GREENLIGHT_UMAMI_WEBSITE_ID=abc123 # dev\n"))
...
func TestParseDotEnvPreservesHashesInQuotedValues(t *testing.T) {
	values := parseDotEnv([]byte("GREENLIGHT_UMAMI_WEBSITE_ID=\"abc # dev\"\n"))
