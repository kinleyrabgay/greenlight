...
func TestPRStep_BitbucketUsesProcessEnvWhenStepEnvIsNil(t *testing.T) {
	dir, baseSHA, headSHA := setupGitRepo(t)
	api := newFakeBitbucketPRAPI(t, 0, "")
	t.Setenv("GREENLIGHT_BITBUCKET_EMAIL", "test@example.com")
	t.Setenv("GREENLIGHT_BITBUCKET_API_TOKEN", "test-token")
	t.Setenv("GREENLIGHT_BITBUCKET_API_BASE_URL", api.server.URL)
