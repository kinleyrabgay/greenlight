...
func TestBuildPipelineSummary_AllClean(t *testing.T) {
	...
	if !strings.Contains(md, "[git push greenlight](https://github.com/kinleyrabgay/greenlight)") {
...
func TestBuildTestingSummaryForPR_RendersLocalTempVisualArtifactPath(t *testing.T) {
	t.Parallel()
	repoRoot := t.TempDir()
	localPath := filepath.Join(os.TempDir(), "greenlight-evidence", "run-123", "checkout.png")
