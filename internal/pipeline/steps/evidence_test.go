...
func TestResolveTestEvidenceDir_DefaultUsesTempRunID(t *testing.T) {
	got := resolveTestEvidenceDir("/work/tree", "feature/foo", "run-123", config.Evidence{StoreInRepo: false, Dir: ".greenlight/evidence"})
	want := filepath.Join(os.TempDir(), "greenlight-evidence", "run-123")
	if got != want {
...
func TestResolveTestEvidenceDir_InRepoKeyedByBranch(t *testing.T) {
	got := resolveTestEvidenceDir("/work/tree", "feature/add-login", "run-123", config.Evidence{StoreInRepo: true, Dir: ".greenlight/evidence"})
	want := filepath.Join("/work/tree", ".greenlight", "evidence", "feature", "add-login")
	if got != want {
