...
func TestTestStep_FixMode(t *testing.T) {
	...
	if got := lastCommitMessage(t, dir); got != "greenlight(test): fix test failures" {
...
func TestTestStep_UserIntentRunsConfiguredCommandThenEvidenceAgent(t *testing.T) {
	...
	for _, want := range []string{
		...
		filepath.Join(os.TempDir(), "greenlight-evidence", sctx.Run.ID),
	...
	if _, err := os.Stat(filepath.Join(os.TempDir(), "greenlight-evidence", sctx.Run.ID)); err != nil {
...
func TestTestStep_InRepoEvidenceFallsBackWhenConfiguredDirEscapesWorktree(t *testing.T) {
	...
	prompt := ag.calls[0].Prompt
	wantDir := filepath.Join(os.TempDir(), "greenlight-evidence", sctx.Run.ID)
