// Package skill holds the canonical content of the greenlight agent skill.
//
...
// Markdown() to the public skills/greenlight/SKILL.md (verified fresh in
...
// installed directory so the agent exposes it as the /greenlight command.
...
const Description = "Validate your code changes through the greenlight pipeline - automated code review, tests, lint, docs, push, PR, and CI - before they reach upstream. Use when the user asks to run greenlight, gate or ship or validate their changes, push safely, asks you to do a task and then validate it, or invokes /greenlight."
...
// tools, e.g. `npx skills add kinleyrabgay/greenlight`) and the copy init
...
` + "`greenlight`" + ` is a local gate that validates your code changes through a pipeline
(intent, rebase, review, test, document, lint, push, PR, CI) before they reach
upstream. You drive it through the ` + "`greenlight axi`" + ` command family, which prints
machine-readable [TOON](https://toonformat.dev) to stdout and progress to stderr.

When the user invokes ` + "`/greenlight`" + `, report the outcome at the end. If the user
...
` + "`greenlight axi run --help`" + ` to see the available flags.

## Two ways to invoke

` + "`/greenlight`" + ` works in two modes, depending on whether the user hands you a
task along with the command:

- **Validate-only** - bare ` + "`/greenlight`" + ` (optionally with flag-style requests
  like "skip the lint step"). The user's code changes are already committed;
  validate them and report the outcome.
- **Task-first** - ` + "`/greenlight <task>`" + `, e.g.
  ` + "`/greenlight add a --json flag to the status command`" + `. First carry out the
...
- The repository must already be initialized with ` + "`greenlight init`" + `.
...
repository is not initialized, run ` + "`greenlight init`" + ` first; if the ` + "`greenlight`" + `
command itself is missing or misbehaving, ` + "`greenlight doctor`" + ` reports what is
wrong.
Before starting, run ` + "`greenlight axi`" + ` (home view).
If it shows an active run on your current branch, resume it or use ` + "`axi abort`" + ` before starting over.
If it shows an active run on another branch, leave that run alone and start validation for your current branch with ` + "`greenlight axi run --intent \"...\"`" + `.
...
directly - greenlight uses it verbatim instead of inferring it from local agent
...
1. Start the run. It blocks until the first decision point or the end:
   ` + "```sh" + `
   greenlight axi run --intent "<what the user set out to accomplish>"
   ` + "```" + `
   ...
   disturbing the run, use ` + "`greenlight axi status`" + ` from a separate call.
2. If the output contains a ` + "`gate:`" + ` object, the pipeline is waiting on you.
   ...
   greenlight axi respond --action fix --findings <id1,id2> --instructions "<optional guidance>"
...
3. Repeat step 2 until the output has an ` + "`outcome:`" + ` instead of a ` + "`gate:`" + `. The
   ...
   - ` + "`checks-passed`" + ` - the change is validated and CI is green, but the PR is
     ...
     (the PR link is in the ` + "`help`" + ` line). greenlight keeps monitoring the PR
   ...
   - ` + "`failed`" + ` or ` + "`cancelled`" + ` - they did not; read the output and address it.
     ...
     pipeline again - ` + "`greenlight axi run --intent \"...\"`" + ` starts a fresh run,
     or ` + "`greenlight rerun`" + ` re-runs the pipeline for the current branch. Do not
...
` + "```sh" + `
greenlight axi               # home view: current branch, active runs, next steps
greenlight axi status        # full detail of the resolved run
greenlight axi logs --step <name> --full   # full log output of one step
greenlight axi abort         # cancel the current-branch active run
` + "```" + `
...
findings[2]{id,severity,file,description,action}:
  ...
  r2,high,cmd/greenlight/main.go,New --force flag bypasses the confirm prompt,ask-user
help[2]:
  greenlight axi respond --action fix --findings r1
  greenlight axi respond --action approve
` + "```" + `
