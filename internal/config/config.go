...
// GlobalConfig represents ~/.greenlight/config.yaml.
...
const defaultConfigYAML = `# greenlight global configuration
...
# User-intent extraction. When you push a branch, greenlight can read recent
...
func (c *Config) ResolveAgent(ctx context.Context, lookPath func(string) (string, error)) error {
	...
	return fmt.Errorf("no supported agent found in PATH (looked for: %s); install one or set 'agent' in ~/.greenlight/config.yaml", strings.Join(probed, ", "))
}
...
// reservedAgentArgs lists flags that greenlight manages internally and that
...
func validateAgentArgsOverride(override map[string][]string) error {
	for name, args := range override {
		...
		for i, arg := range args {
			...
			if reserved[base] {
				return fmt.Errorf("invalid agent_args_override.%s[%d]: %q is managed by greenlight and cannot be overridden", name, i, arg)
			}
		}
	}
	return nil
...
// LoadRepo reads per-repo config from dir/.greenlight.yaml.
// Returns zero-value config if file doesn't exist.
func LoadRepo(dir string) (*RepoConfig, error) {
	cfg := &RepoConfig{}

	path := filepath.Join(dir, ".greenlight.yaml")
	data, err := os.ReadFile(path)
	if err != nil {
		if errors.Is(err, fs.ErrNotExist) {
			return cfg, nil
...
// opt-in (off by default); when enabled it lands under .greenlight/evidence.
