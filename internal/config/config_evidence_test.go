...
func TestTestEvidenceDefaults(t *testing.T) {
	...
	if got.Evidence.Dir != ".greenlight/evidence" {
		t.Errorf("default Dir = %q, want .greenlight/evidence", got.Evidence.Dir)
	}
}
...
func TestTestEvidenceMerge_GlobalEnable(t *testing.T) {
	...
	if cfg.Test.Evidence.Dir != ".greenlight/evidence" {
...
test:
  evidence:
    store_in_repo: true
`
	if err := os.WriteFile(filepath.Join(dir, ".greenlight.yaml"), []byte(yaml), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
