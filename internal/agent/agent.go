...
// are injected into the underlying tool's argv ahead of greenlight' managed flags.
...
func NewWithOptions(name types.AgentName, bin string, extraArgs []string, opts Options) (Agent, error) {
	...
	default:
		return nil, fmt.Errorf("unknown agent %q; valid options: auto, claude, codex, rovodev, opencode, pi, acp:<target> (set 'agent' in ~/.greenlight/config.yaml)", name)
	}
}
