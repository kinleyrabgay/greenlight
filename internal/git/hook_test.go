...
func TestPostReceiveHookScript(t *testing.T) {
	script := postReceiveHookScript("/opt/Greenlight/greenlight")
	...
	if !strings.Contains(script, "greenlight") {
...
func TestShellSingleQuote(t *testing.T) {
	tests := []struct {
		...
		{"plain", "/usr/bin/greenlight", "'/usr/bin/greenlight'"},
		{"spaces", "/opt/Greenlight/bin", "'/opt/Greenlight/bin'"},
...
func TestPostReceiveHookScriptDoesNotEvaluatePushOptions(t *testing.T) {
	...
	fakeBin := filepath.Join(base, "fake-greenlight")
...
func TestPostReceiveHook_SurfacesNotifyFailures(t *testing.T) {
	...
	// Fake greenlight binary that always fails notify-push with a
