...
while read oldrev newrev refname; do
  ...
  if [ $status -ne 0 ]; then
    ...
    {
      printf 'greenlight: notify-push failed for %s (exit %d):\n' "$refname" "$status"
      printf '%s\n' "$out"
...
func isManagedPostReceiveHook(content []byte) bool {
	text := string(content)
	return strings.Contains(text, "# greenlight post-receive hook") && strings.Contains(text, "daemon notify-push")
}
...
// RefreshManagedPostReceiveHook updates an existing greenlight-owned hook.
