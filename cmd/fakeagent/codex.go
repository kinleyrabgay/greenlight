...
func runCodex(args []string, scenario *Scenario) int {
	...
	// to only fields declared in the schema. Otherwise greenlight'
	...
	// Replay recorded codex output if a fixture is available. greenlight
...
// real. greenlight parses JSON out of the agent_message text, so for
...
// mirroring that behaviour keeps the fake consistent with greenlight'
