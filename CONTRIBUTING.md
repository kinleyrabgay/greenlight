...
**All pull requests to this repository must be raised through `greenlight`.**

This repo _is_ greenlight. Contributions should be done using the tool itself, which reduces the maintainer's burden of reviewing and merging contributions.
A GitHub Actions check (`Require greenlight`) runs on every PR and fails if the body is missing the deterministic signature that greenlight writes. PRs without it will not be reviewed or merged.

## Workflow

1. Fork the repo and clone your fork.
2. Create a branch and make your changes.
3. Initialize or refresh the gate in the repo: `greenlight init`.
...
6. Run `greenlight` to attach to the pipeline, watch findings, and auto-fix or review as needed.
...
- Run `make skill` when you change the canonical agent skill content under `internal/skill`; `make lint` fails if any committed greenlight skill file has drifted.
