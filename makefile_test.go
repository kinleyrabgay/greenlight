...
func TestMakeBuildPrioritizesDotEnvUmamiWebsiteID(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(workDir, ".env"), []byte("GREENLIGHT_UMAMI_WEBSITE_ID=website-from-dotenv\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestMakeBuildEmbedsDefaultSelfHostedTelemetryConfig(t *testing.T) {
	...
	if !strings.Contains(output, "TelemetryHost=https://a.kinleyrabgay.com") {
...
func TestMakeBuildPrioritizesDotEnvUmamiHost(t *testing.T) {
	...
	workDir := writeTestMakeWorkspace(t)
	if err := os.WriteFile(filepath.Join(workDir, ".env"), []byte("GREENLIGHT_UMAMI_HOST=https://dotenv.example\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestMakeBuildStripsInlineCommentsFromDotEnvUmamiWebsiteID(t *testing.T) {
	...
	workDir := writeTestMakeWorkspace(t)
	if err := os.WriteFile(filepath.Join(workDir, ".env"), []byte("GREENLIGHT_UMAMI_WEBSITE_ID=website-from-dotenv # dev\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestMakeBuildPreservesQuotedHashInDotEnvUmamiWebsiteID(t *testing.T) {
	...
	workDir := writeTestMakeWorkspace(t)
	if err := os.WriteFile(filepath.Join(workDir, ".env"), []byte("GREENLIGHT_UMAMI_WEBSITE_ID=\"website # dev\"\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func runMakeDryBuild(t *testing.T, makePath, workDir string, extraEnv map[string]string) string {
	...
	cmd.Dir = workDir
	cmd.Env = filteredEnv(os.Environ(), "UMAMI_HOST", "UMAMI_WEBSITE_ID", "GREENLIGHT_UMAMI_HOST", "GREENLIGHT_UMAMI_WEBSITE_ID")
