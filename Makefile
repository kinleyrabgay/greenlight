...
DEFAULT_UMAMI_HOST := https://a.kinleyrabgay.com
DEFAULT_UMAMI_WEBSITE_ID := f959e889-92f5-4121-8a1f-571b10861198
DOTENV_UMAMI_HOST := $(shell [ -f .env ] && perl -ne 'next if /^\s*(?:\#|$$)/; s/^\s*export\s+//; next unless /^\s*GREENLIGHT_UMAMI_HOST\s*=\s*(.*)$$/; $$v=$$1; $$v =~ s/^\s+|\s+$$//g; if ($$v =~ /^( ["\x27] )(.*)\1$$/x) { $$v=$$2; } else { $$v =~ s/\s+\#.*$$//; $$v =~ s/\s+$$//; } $$out=$$v; END { print $$out if defined $$out }' .env)
DOTENV_UMAMI_WEBSITE_ID := $(shell [ -f .env ] && perl -ne 'next if /^\s*(?:\#|$$)/; s/^\s*export\s+//; next unless /^\s*GREENLIGHT_UMAMI_WEBSITE_ID\s*=\s*(.*)$$/; $$v=$$1; $$v =~ s/^\s+|\s+$$//g; if ($$v =~ /^(["\x27])(.*)\1$$/) { $$v=$$2; } else { $$v =~ s/\s+\#.*$$//; $$v =~ s/\s+$$//; } $$out=$$v; END { print $$out if defined $$out }' .env)
...
LDFLAGS := -X github.com/kinleyrabgay/greenlight/internal/buildinfo.Version=$(VERSION) \
           -X github.com/kinleyrabgay/greenlight/internal/buildinfo.Commit=$(COMMIT) \
           -X github.com/kinleyrabgay/greenlight/internal/buildinfo.Date=$(DATE) \
...
build:
	go build -ldflags "$(LDFLAGS)" -o bin/greenlight ./cmd/greenlight
...
dist:
	...
	for target in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		CGO_ENABLED=0 GOOS="$$os" GOARCH="$$arch" go build -ldflags "$(LDFLAGS)" -o "$$out" ./cmd/greenlight; \
		if [ "$$os" = "windows" ]; then \
			( cd "$(DIST_DIR)" && zip -q "greenlight-$(VERSION)-$$os-$$arch.zip" "$$bin" ); \
		else \
			tar -C "$(DIST_DIR)" -czf "$(DIST_DIR)/greenlight-$(VERSION)-$$os-$$arch.tar.gz" "$$bin"; \
		fi; \
		rm -f "$$out"; \
	done
...
install: build
	...
	install -m 755 bin/greenlight $(INSTALL_BIN)
...
# End-to-end suite: drives the real greenlight binary against a fake
...
# Regenerate the committed agent skill (skills/greenlight/SKILL.md) from the
